...
module.exports = function SalchyAutoCast(script) {
	...
	script.command.add('cast', () => {
		enabled = !enabled;
		script.command.message(`自动施法脚本已 -  ${(enabled) ? '开启' : '关闭'}.`);
	})
	script.command.add('castpl', () => {
		autoplague = !autoplague;
		script.command.message(`自动破甲现在已 -  ${(autoplague) ? '开启' : '关闭'}.`);
	})
	script.command.add('castpu', () => {
		autopull = !autopull;
		script.command.message(`自动拉取现在已 -  ${(autopull) ? '开启' : '关闭'}.`);
	})
	script.command.add('castpvp', () => {
		pvp = true;
		pve = false;
		script.command.message(`开启PVP模式.`);
	})
	script.command.add('castpve', () => {
		pve = true;
		pvp = false;
		script.command.message(`开启PVE模式.`);
	})
	script.command.add('castplayer', (arg) => {
		if(arg){
		players_to_focus = [];
		players_to_focus.push(arg);
		script.command.message("通知 " + arg);
		} else {
		focusplayer = !focusplayer;
		script.command.message(`通知已- ${(focusplayer) ? '开启' : '关闭'}.`);
		}
	})
